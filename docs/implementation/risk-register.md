...
| R-002 | Clean PostgreSQL migration/readiness has no live evidence for the latest migration. | Closed | Host-terminal `make compose-smoke` migrated a clean Compose database to `0003_run_dispatch`; all four PostgreSQL integration tests and health checks passed, then scripted cleanup removed the environment. | Reopen if a later migration lacks clean-database evidence. |
