...
| R-003 | GitHub Actions and Trivy have not executed remotely. | Open | One main-only CI job reuses one Compose build; local pip-audit and production npm audit pass. The image uses updated Debian/Python bases and removes unneeded runtime build tooling after dependency installation. The HIGH/CRITICAL Trivy gate has two third-party-SBOM-only suppressions that expire 2026-09-15; Gitleaks owns secret scanning. | Observe a successful main workflow run and revalidate/remove the temporary suppressions before expiry. |
